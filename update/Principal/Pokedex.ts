...
import { explorar, type DistribuicaoRaridade, defaultRng, type RNG, type ResultadoEncontro } from "../Game/Exploracao";
...
export class Pokedex {
    ...
    private _ordemNomes: string[] = []; // para renderizar índices estáveis

    private _encontroAtual: EstadoEncontro | null = null;

    private _cooldownExplorarMs = 15_000;
    private _ultimaExploracaoTs = 0;

    constructor() {}

    /** Compatibilidade: mantêm a ideia de "tamanho" como quantidade de Pokémon únicos na coleção */
    getTamanho(): number {
    ...
    /** Compatibilidade para testes antigos: agora usa o banco real e converte para instância com Atributos */
    ...
    /** Adiciona uma instância manual (usado pelos testes antigos). */
    ...
    capturar(rng: RNG = defaultRng, modificador?: number): ResultadoCaptura | { erro: string } {
        ...
        // se já foi capturado no mesmo encontro, não tenta de novo
        if (this._encontroAtual.capturado) {
            return { capturado: false, mensagem: "Este Pokémon já foi capturado nesta tentativa." };
        }
        ...
        if (resultado.capturado) {
            ...
            if (this._colecao.has(chave)) {
                ...
                // mantém a instância antiga (para treinos/batalhas preservarem energia etc.)
    ...
    private criarInstanciaAPartirDoBanco(dados: DadosPokemon): Pokemon {
        // Ao capturar, criamos instância com energia aleatória
    ...
    // -------------------------
    // Mantém gameplay existente (treino/batalha/descanso)
    // -------------------------
    ...
    // Opcional: mantém nome antigo (compat) para Main.ts antigo se precisar
